...
orderElement.innerHTML +=`<h2>Order ${order._id}</h2>

<p><b>Status:</b> <span style="color:${statusColor}">${order.status}</span></p> <p><b>Customer name: </b> ${order.name}</p>
...
orderElement.innerHTML +=`<p><img height="50" src="${p.photo_url}"> ${p.name} |${p.price}$</p>`;

sum +=+p.price

orderElement.innerHTML +=`<p>Total Price: ${sum}</p>
